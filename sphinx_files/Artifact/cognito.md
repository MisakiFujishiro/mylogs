...
ここの設定を行うことで、Cognitoにと合わせがしてくるクライアントを定義し、そのクライアントIDが払い出される。
...
- OAuth2.0のフローとスコープ  
    ...
    - スコープ  
        アプリケーションがアクセストークンを要求して、そのアクセストークンでアプリケーションが使用できるAPIリソースの範囲を定義するための設定  
        OAuth2.0では、保護されたリソースに対するアクセス制御する方法としてスコープという概念を使用している。  
        Cognitoが保護されたリソースへのアクセス権を指定する。（認可の指定)    
        今回は以下を許可する
        - openid: IDトークンを作成するための権限が付与される
        - aws.cognito.signin.user.admin: 管理者権限を持つアプリケーションが他のユーザーのアカウントにアクセスするために使用されます。このスコープを持つアプリケーションは、他のユーザーのパスワードをリセットしたり、他のユーザーのアカウントを管理することができます。
        - profile: ユーザーの基本プロフィール情報にアクセスするために使用されます。
