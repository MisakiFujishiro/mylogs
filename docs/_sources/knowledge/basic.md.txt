...
仕様は、外部と関わる部分の「こうなるはず」という完成図（実装非依存）

設計や、どうやって作るのかを定義するもの（実装に紐づくもの）
...
# DDD
## なぜDDD
ドメインがビジネスの根幹なので、最終的に動くプログラムの価値に直結する。だから大事

## DDDって何
Eric Evansという人が公開した開発でうまくいったもの。
DDDはあくまで、ある成功パターンに過ぎないので、案件パターンに合わせて似たような結論に至っていることもある。

GRASPとかデザインパターンとか責務駆動設計とかの知識を踏まえた上でそれらをうまくいったものを整理したのがDDD