...
エッジロケーションのサービスを利用することで、以下の課題を計喝することができる
- 地理的距離による遅延
- 大量のアクセスへの対処
- DDos攻撃
...
- ビヘイビア  
  `パスの設定をするようなもの`  
  キャッシュポリシーでエッジサーバーの振る舞いに関する設定が可能。  
  すなわち、パスごとにキャッシングルールなどを設定することができる。  
  リクエストは全てCloudFrontでフィルタリングされるので、ビヘイビアの中で明示的にヘッダー、クッキー、クエリストリングなどの情報をオリジンに渡すかを設定する。
- オリジン  
  `バックエンドのオリジンを指定するもの`  
